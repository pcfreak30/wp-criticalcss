	...
	public function print_styles() {
		...
		if ( ! get_query_var( 'nocache' ) && ! is_404() ) {
			...
			if ( 'on' === $this->plugin->settings_manager->get_setting( 'template_cache' ) && ! empty( $type['template'] ) ) {
				if ( empty( $cache ) ) {
					if ( ! $this->plugin->model_manager->API_Queue->get_item_exists( $type ) ) {
						$this->plugin->queue->schedule_api_task( $type );
					}
				}
			} else {
				$check = $this->plugin->cache_manager->get_cache_fragment( [ 'webcheck', $hash ] );
				if ( ! $manual && ( empty( $check ) || ( ! empty( $check ) && empty( $cache ) && null !== $cache ) ) && ! $this->plugin->model_manager->Web_Queue->get_item_exists( $type ) ) {
					$this->plugin->queue->schedule_web_task( $type );
